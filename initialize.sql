...
\copy Products(product_id, product_name, price, amount, category, seller_id) FROM products.csv csv header;

\copy Transactions(transaction_id,date,price,amount,customer_id,product_id) FROM transactions.csv csv header;

\copy Auctions(transaction_id, min_auc_price, time_duration) FROM auctions.csv csv header;

\copy Bids(bid_id, auction_id, amount) FROM bids.csv csv header;
